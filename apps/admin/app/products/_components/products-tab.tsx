...
export function ProductsTab({ state }: Props) {
  ...
  const { currentPage, setCurrentPage, totalPages, pageSize, setPageSize, pageSizeOptions, startIndex, endIndex } = usePersistedPagination({
    storageKey: "admin:pagination:products",
    totalItems: filteredProducts.length,
    pageSizeOptions: [20, 50, 100],
    resetDeps: [searchQuery, activeFilter],
  });
  ...
  async function executeConfirmedAction() {
    ...
    try {
      if (confirmAction.kind === "create") {
        ...
        if (confirmAction.selectedImageFile) {
          const { url } = await uploadAdminAssetApi(confirmAction.selectedImageFile, "products", String(form.id));
          imageUrl = url;
        }
  ...
  async function submitEditProduct(event: FormEvent<HTMLFormElement>) {
    ...
    setConfirmAction({
      ...
      id: editingForm.id,
      form: editingForm,
      selectedImageFile: editingForm.selectedImageFile || null,
    });
    setConfirmError(null);
  }
  ...
  function handleEditImageSelect(event: ChangeEvent<HTMLInputElement>) {
    ...
    setEditingForm((prev) => (prev ? { ...prev, selectedImageFile: file || null } : prev));
            ...
            <form onSubmit={submitEditProduct} className="mt-4 space-y-3">
              ...
              <label className="block space-y-1">
                ...
                <input
                  type="file"
                  accept="image/*"
                  onChange={(event) => void handleEditImageSelect(event)}
                ...
                <p className="text-[11px] text-stone-500">
                  {uploadingEditImage ? "이미지 업로드 중..." : editingForm.selectedImageFile ? `선택됨: ${editingForm.selectedImageFile.name}` : "이미지 파일을 선택해주세요."}
                </p>
