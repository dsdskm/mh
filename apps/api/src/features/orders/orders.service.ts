...
export class OrdersService {
  ...
  async updateOrderStatus(id: string, status: OrderStatus): Promise<Order> {
    const updated = await this.dataSource.transaction(async (manager) => {
      ...
      if (currentStatus === ORDER_STATUS.CANCELLED && status !== ORDER_STATUS.CANCELLED) {
      ...
      return orderRepository.save({
        ...order,
        status,
        cancelReason: status === ORDER_STATUS.CANCELLED ? order.cancelReason : null,
      });
    });
  ...
  private normalizeOrderStatus(statusRaw: string): OrderStatus {
    ...
    if (statusRaw === ORDER_STATUS.CANCELLED || statusRaw === '취소') {
